...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._buf_seq = {}
        self._buf_seq_tb = {}
        self._buf_seq_ltd = {}  # v7/C3: continuous log(1 + time_delta) per event
        self._buf_seq_lens = {}
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for domain in self.seq_domains:
            ...
            # Time bucketing (v0 path) + continuous log-Δt (v7/C3 path).
            # Both are emitted unconditionally; the model picks which to
            # consume based on use_continuous_time flag.
            ...
            if ts_ci is not None and ts_offs is not None and ts_vals is not None:
                ...
                # v7/C3: continuous log(1 + Δt). Padding positions get 0 so
                # the bias-free ContinuousTimeEncoder emits exactly the zero
                # vector at padding (fixes v4's LayerNorm-bias leak).
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    valid_rows = sum(r[2] for r in rg_info[full_train_end:])
    ...
    valid_dataset = PCVRParquetDataset(
        ...
        shuffle=False,
        buffer_batches=0,
        row_group_range=(full_train_end, total_rgs),
        clip_vocab=clip_vocab,
        seq_truncate=seq_truncate,
    )
