...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            row_group_range: ``(start, end)`` slice of Row Groups; ``None`` to
                use all Row Groups. Ignored when ``row_groups`` is provided.
            row_groups: explicit ``(file_path, rg_idx, num_rows)`` list in the
                desired iteration order. When set, replaces the auto-built list
                entirely; used by ``get_pcvr_data`` to pass a timestamp-sorted
                split. ``None`` falls back to the glob-order auto build.
            ...
            seq_truncate: sequence truncation policy. ``head`` keeps the first
                tokens, ``tail`` keeps the last tokens, and ``auto`` infers the
                timestamp order per domain and keeps the most recent tokens.
        """
        super().__init__()
        if seq_truncate not in {'auto', 'head', 'tail'}:
            raise ValueError(
                f"seq_truncate must be one of auto/head/tail, got {seq_truncate!r}")
        ...
        self.seq_truncate = seq_truncate
        self._seq_truncate_policy: Dict[str, str] = {}
        ...
        self._validate_columns(schema_names)
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        with open(schema_path, 'r', encoding='utf-8') as f:
            raw = json.load(f)
        required_keys = {'user_int', 'item_int', 'user_dense', 'seq'}
        missing_keys = sorted(required_keys.difference(raw))
        if missing_keys:
            raise KeyError(
                f"schema.json missing required keys: {missing_keys}")
    ...
    def _validate_columns(self, schema_names: List[str]) -> None:
        """Fail early when the schema references columns absent from Parquet."""
        available = set(schema_names)
        required = {'timestamp', 'user_id'}
        if self.is_training:
            required.add('label_type')

        required.update(f'user_int_feats_{fid}' for fid, _, _ in self._user_int_cols)
        required.update(f'item_int_feats_{fid}' for fid, _, _ in self._item_int_cols)
        required.update(f'user_dense_feats_{fid}' for fid, _ in self._user_dense_cols)
        for domain in self.seq_domains:
            prefix = self._seq_prefix[domain]
            required.update(f'{prefix}_{fid}' for fid in self.seq_feature_ids[domain])

        missing = sorted(required.difference(available))
        if missing:
            preview = ', '.join(missing[:30])
            suffix = '' if len(missing) <= 30 else f', ... ({len(missing)} total)'
            raise KeyError(
                "Parquet file is missing columns required by schema.json: "
                f"{preview}{suffix}")
    ...
    def _resolve_seq_truncate(
        self,
        domain: str,
        ts_offs: Optional["npt.NDArray[np.int64]"],
        ts_vals: Optional["npt.NDArray[np.int64]"],
        B: int,
    ) -> str:
        """Resolve ``auto`` into ``head`` or ``tail`` for a sequence domain."""
        if self.seq_truncate in {'head', 'tail'}:
            return self.seq_truncate
        if domain in self._seq_truncate_policy:
            return self._seq_truncate_policy[domain]
        ...
        policy = 'tail' if ascending >= descending else 'head'
        self._seq_truncate_policy[domain] = policy
        logging.info(
            f"seq_truncate=auto resolved {domain} to {policy} "
            f"(ascending_rows={ascending}, descending_rows={descending})")
        return policy

    @staticmethod
    def _sequence_start(raw_len: int, use_len: int, policy: str) -> int:
        """Return the source offset inside one raw sequence after truncation."""
        if policy == 'tail' and raw_len > use_len:
            return raw_len - use_len
        return 0
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        if self.is_training:
            ...
            # Auxiliary click label used by the multi-task head (label_type 1
            # is a click without conversion; 2 is a click followed by
            # conversion). Always emitted at the dataset boundary; the
            # trainer chooses to use it based on aux_click_weight.
            labels_click = ((label_type_arr == 1) | (label_type_arr == 2)).astype(np.int64)
        else:
            labels = np.zeros(B, dtype=np.int64)
            labels_click = np.zeros(B, dtype=np.int64)
        user_ids = batch.column(self._col_idx['user_id']).to_pylist()
        item_ids = (batch.column(self._col_idx['item_id']).to_pylist()
                    if 'item_id' in self._col_idx else [])
        ...
        result = {
            ...
            'label_click': torch.from_numpy(labels_click),
        ...
        for domain in self.seq_domains:
            ...
            trunc_policy = self._resolve_seq_truncate(domain, ts_offs, ts_vals, B)
            ...
            for c, (offs, vals, vs, ci) in enumerate(col_data):
                for i in range(B):
                    ...
                    if rl <= 0:
                        continue
                    ul = min(rl, max_len)
                    rel_start = self._sequence_start(rl, ul, trunc_policy)
                    src_start = s + rel_start
            ...
            time_bucket[:] = 0
            if ts_ci is not None and ts_offs is not None and ts_vals is not None:
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    Row groups are sorted by their ``timestamp`` column min, then the last
    ``valid_ratio`` fraction is taken as validation. This is a temporally
    causal split — needed because the platform's hidden test set is later
    in time than training, so a row-group-order split (which depends on
    glob order, not time) leaves a leaky validation signal.

    The function falls back to glob order with a warning if any row group
    is missing timestamp statistics.
    ...
    # Build (file, rg_idx, num_rows, ts_min, ts_max) tuples, reading per-row-
    # group timestamp statistics from the parquet metadata. This is metadata-
    # only (no row scan), so cost is negligible on the full training set.
    rg_info: List[Tuple[str, int, int, Optional[int], Optional[int]]] = []
    missing_stats = 0
    for f in pq_files:
        pf = pq.ParquetFile(f)
        try:
            ts_col = pf.schema_arrow.names.index('timestamp')
        except ValueError:
            ts_col = -1
        for i in range(pf.metadata.num_row_groups):
            ...
            if ts_col >= 0:
                stats = rg.column(ts_col).statistics
                if stats is not None and stats.has_min_max:
                    ts_min = int(stats.min)
                    ts_max = int(stats.max)
            if ts_min is None:
                missing_stats += 1
            rg_info.append((f, i, rg.num_rows, ts_min, ts_max))

    total_rgs = len(rg_info)
    if total_rgs < 2:
        raise ValueError(
            f"Need at least 2 Parquet Row Groups for train/valid split, got {total_rgs}. "
            "For the Hugging Face demo sample, run tools/prepare_hf_sample.py "
            "to rewrite the file with multiple Row Groups.")

    if missing_stats > 0:
        logging.warning(
            f"{missing_stats}/{total_rgs} row groups missing timestamp statistics; "
            "falling back to glob order (validation split may not be temporally causal). "
            "Re-run the parquet writer with statistics enabled to fix.")
    else:
        # Stable sort by ts_min so ties (same minute) preserve original order.
        rg_info.sort(key=lambda r: (r[3], r[0], r[1]))

    n_valid_rgs = max(1, int(total_rgs * valid_ratio))
    n_valid_rgs = min(n_valid_rgs, total_rgs - 1)
    full_train_end = total_rgs - n_valid_rgs
    n_train_rgs = full_train_end

    # train_ratio: use only the first N% of the training Row Groups (after
    # the temporal sort, this is the temporally earliest portion).
    if train_ratio < 1.0:
        n_train_rgs = max(1, int(full_train_end * train_ratio))
        logging.info(
            f"train_ratio={train_ratio}: using {n_train_rgs}/{full_train_end} "
            "pre-validation train Row Groups")

    train_rgs = rg_info[:n_train_rgs]
    valid_rgs = rg_info[full_train_end:]
    train_rows = sum(r[2] for r in train_rgs)
    valid_rows = sum(r[2] for r in valid_rgs)
    unused_rows = sum(r[2] for r in rg_info[n_train_rgs:full_train_end])

    logging.info(f"Row Group split: {n_train_rgs} train ({train_rows} rows), "
                 f"{n_valid_rgs} valid ({valid_rows} rows), "
                 f"{unused_rows} unused due to train_ratio")

    if missing_stats == 0 and train_rgs and valid_rgs:
        train_max_ts = max(r[4] for r in train_rgs if r[4] is not None)
        valid_min_ts = min(r[3] for r in valid_rgs if r[3] is not None)
        if train_max_ts > valid_min_ts:
            logging.warning(
                f"Train/valid temporal overlap: max(train.ts_max)={train_max_ts} > "
                f"min(valid.ts_min)={valid_min_ts}. Within-row-group time spread "
                "exceeds the gap between groups; validation will be partially "
                "non-causal but still strictly later on average than training.")
        else:
            logging.info(
                f"Train/valid temporally disjoint: train ends {train_max_ts}, "
                f"valid starts {valid_min_ts}.")

    # Strip the timestamp fields before passing to the dataset constructor.
    train_rg_list = [(r[0], r[1], r[2]) for r in train_rgs]
    valid_rg_list = [(r[0], r[1], r[2]) for r in valid_rgs]
