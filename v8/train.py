...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--seq_truncate', type=str, default='auto',
                        choices=['auto', 'head', 'tail'],
                        help='Sequence truncation policy. auto keeps the most '
                             'recent tokens based on per-domain timestamp order.')
    ...
    parser.add_argument('--use_continuous_time', action='store_true', default=False,
                        help='v7/C3: REPLACE the bucket time embedding with a '
                             'bias-free ContinuousTimeEncoder applied to '
                             'log(1 + (label_ts - event_ts)). Mutually exclusive '
                             'with the bucket path. Bucket embedding is not '
                             'constructed when this flag is on.')
    parser.add_argument('--use_din_query', action='store_true', default=False,
                        help='v8/C1: DIN-style target-aware query generator. '
                             'Replaces MultiSeqQueryGenerator (mean-pool) with '
                             'DINQueryGenerator (per-token attention scored '
                             'against the target item embedding). Output '
                             'signature unchanged; downstream blocks unaffected.')
    ...
    # Platform jobs set these env vars. Defaults make local smoke runs less brittle.
    script_dir = os.path.dirname(os.path.abspath(__file__))
    args.ckpt_dir = args.ckpt_dir or os.path.join(script_dir, 'outputs', 'ckpt')
    args.log_dir = args.log_dir or os.path.join(script_dir, 'outputs', 'logs')
    args.tf_events_dir = args.tf_events_dir or os.path.join(script_dir, 'outputs', 'tf_events')
...
def main() -> None:
    ...
    train_loader, valid_loader, pcvr_dataset = get_pcvr_data(
        ...
        seq_truncate=args.seq_truncate,
    )
    ...
    model_args = {
        ...
        "use_din_query": args.use_din_query,
    }
