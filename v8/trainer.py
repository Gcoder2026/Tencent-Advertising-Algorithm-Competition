...
class PCVRHyFormerRankingTrainer:
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        for domain in seq_domains:
            ...
            # v7/C3: always populate log_time_delta when the dataset emits it.
            # If use_continuous_time=True and this key is missing, the model
            # raises (no silent fallback to zeros — the v4 regression mode).
            key_ltd = f'{domain}_log_time_delta'
            if key_ltd in device_batch:
                seq_log_time_delta[domain] = device_batch[key_ltd]
        return ModelInput(
