...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._buf_seq = {}
        self._buf_seq_tb = {}
        self._buf_seq_tld = {}  # continuous log(1 + time_diff) per sequence event
        self._buf_seq_lens = {}
        for domain in self.seq_domains:
            ...
            self._buf_seq_tld[domain] = np.zeros((B, max_len), dtype=np.float32)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for domain in self.seq_domains:
            ...
            # Time bucketing + continuous log-delta encoding.
            time_bucket = self._buf_seq_tb[domain][:B]
            time_bucket[:] = 0
            time_log_dt = self._buf_seq_tld[domain][:B]
            time_log_dt[:] = 0.0
            if ts_ci is not None:
                ...
                # Continuous log(1 + dt) encoding, complementary to the
                # discrete buckets above. Discrete buckets collapse all
                # intra-bucket ordering (e.g., events at 1s/3s/4.9s all map
                # to bucket 1); log(1 + dt) preserves it as a smooth signal
                # the model can learn from. Padding positions stay at 0.
                log_dt = np.log1p(time_diff.astype(np.float32))
                log_dt[ts_padded == 0] = 0.0
                time_log_dt[:] = log_dt

            result[f'{domain}_time_bucket'] = torch.from_numpy(time_bucket.copy())
            result[f'{domain}_time_log_dt'] = torch.from_numpy(time_log_dt.copy())
