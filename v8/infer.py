...
def _build_model(
    ...
    model_args = {
        ...
        "use_continuous_time": bool(cfg.get("use_continuous_time", False)),
        "use_din_query": bool(cfg.get("use_din_query", False)),
    }
...
def _make_model_input(batch: Dict[str, Any], device: torch.device) -> ModelInput:
    ...
    seq_log_time_delta: Dict[str, torch.Tensor] = {}
    for domain in seq_domains:
        ...
        # v7/C3: populate log_time_delta if the dataset emits it. The model
        # raises if use_continuous_time=True and the field is missing — this
        # is the structural fix for the v4 silent-fallback regression mode.
        key_ltd = f"{domain}_log_time_delta"
        if key_ltd in batch:
            seq_log_time_delta[domain] = batch[key_ltd].to(device, non_blocking=True)
    return ModelInput(
        ...
        seq_log_time_delta=seq_log_time_delta if seq_log_time_delta else None,
    )
