...
class ModelInput(NamedTuple):
    ...
    # v10: cyclical row-level time features. Optional (default None) for
    # back-compat with v0..v9 ModelInput usage. Only consumed when
    # use_cyclical_time=True in the PCVRHyFormer constructor.
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # v10: cyclical row-level time features. When True, adds small
        # hour-of-day (24-slot) and day-of-week (7-slot) embeddings,
        # summed and added to user_dense_tok. Captures wall-clock context
        # the per-event 65-bucket time embedding can't (it measures Δt
        # only). Additive, NOT a replacement — keeps the bucket emb.
        ...
        if use_cyclical_time:
            # Small embeddings. d_model output for both so they can sum
            # cleanly into the user_dense_tok stream.
            self.hour_embedding = nn.Embedding(24, d_model)
            self.day_embedding = nn.Embedding(7, d_model)
            # Initialize small so they don't dominate the trained
            # user_dense_proj output at the start of training.
            nn.init.normal_(self.hour_embedding.weight, std=0.01)
            nn.init.normal_(self.day_embedding.weight, std=0.01)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        if self.has_user_dense:
            ...
            # v10: additive cyclical row-time embedding into user_dense_tok.
            # Hour + day, broadcast to (B, 1, D). Small init keeps the
            # contribution from dominating before training learns useful
            # cyclical patterns.
            if self.use_cyclical_time and inputs.hour_of_day is not None and inputs.day_of_week is not None:
                cyc = self.hour_embedding(inputs.hour_of_day) + self.day_embedding(inputs.day_of_week)
