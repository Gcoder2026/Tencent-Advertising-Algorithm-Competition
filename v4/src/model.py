...
class ModelInput(NamedTuple):
    ...
    # Continuous log(1 + time_diff) per sequence event, [B, L] float32.
    # Optional with default None for back-compat: when the model is built
    # with use_continuous_time=False, this field is ignored entirely, so
    # checkpoints from v1/v2/v3 (which never populate it) still load and
    # run correctly through the same forward path.
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Continuous time encoding (v4): when True, augments the discrete
        # time-bucket embedding with a learned projection of log(1 + dt)
        # added to each sequence-event token. dt is computed in data.py
        # per sequence event as label_time_row - event_time_in_seq.
        ...
        if use_continuous_time:
            # Small MLP projecting log(1 + dt) (scalar) into d_model. The
            # SiLU non-linearity gives the model headroom to learn a non-
            # monotonic recency curve (e.g., very recent events less
            # informative than 30-minute-old ones in some cases). LayerNorm
            # keeps the additive contribution comparable in magnitude to
            # the token embedding it gets added to in _embed_seq_domain.
            self.continuous_time_proj = nn.Sequential(
                nn.Linear(1, d_model),
                nn.SiLU(),
                nn.LayerNorm(d_model),
            )
    ...
    def _embed_seq_domain(
        ...
        # Add continuous log(1 + dt) embedding when enabled. Complements the
        # discrete bucket above by preserving intra-bucket ordering. Padding
        # positions have log_dt=0 (set in data.py) so they contribute the
        # LayerNorm bias term, which we accept as numerically small.
