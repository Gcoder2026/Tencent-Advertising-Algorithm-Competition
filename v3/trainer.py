...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     ...
                     f"aux_click_weight={aux_click_weight}, "
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        logits_all = self.model(model_input)  # (B, action_num)
        # Convention: head 0 = conversion (the leaderboard target);
        # head 1 (when present) = click, used as auxiliary supervision.
        conv_logits = logits_all[..., 0]  # (B,)

        if self.loss_type == 'focal':
            loss = sigmoid_focal_loss(
                conv_logits, label, alpha=self.focal_alpha, gamma=self.focal_gamma)
        else:
            loss = F.binary_cross_entropy_with_logits(conv_logits, label)

        # Multi-task auxiliary click loss (only when the model has a second head
        # AND the dataset emitted label_click AND the user weight is non-zero).
        if (logits_all.dim() >= 2 and logits_all.size(-1) >= 2
                and self.aux_click_weight > 0.0
                and 'label_click' in device_batch):
            click_logits = logits_all[..., 1]  # (B,)
            label_click = device_batch['label_click'].float()
            loss_click = F.binary_cross_entropy_with_logits(click_logits, label_click)
            loss = loss + self.aux_click_weight * loss_click
    ...
    def _evaluate_step(
        ...
        Validation AUC is always computed against the conversion head
        (the leaderboard metric), regardless of how many auxiliary heads
        the multi-task model carries.
        """
        device_batch = self._batch_to_device(batch)
        label = device_batch['label']

        model_input = self._make_model_input(device_batch)
        logits_all, _ = self.model.predict(model_input)  # (B, action_num), (B, D)
        logits = logits_all[..., 0]  # conversion logit, (B,)
