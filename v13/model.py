...
class ModelInput(NamedTuple):
    ...
    # v13: cyclical row-level time features. Optional (default None)
    # for back-compat. v13 trainer/infer always populate them.
    hour_of_day: Optional[torch.Tensor] = None   # (B,) int64, 0-23
    day_of_week: Optional[torch.Tensor] = None   # (B,) int64, 0-6
    month_of_year: Optional[torch.Tensor] = None # (B,) int64, 0-11
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # v13: cyclical row-level time features (hour-of-day + day-of-week
        # + month-of-year). Per 小红书 community consensus, this gives
        # ~+0.01 AUC. UNCONDITIONAL embedding modules — they always exist
        # in the model graph and state-dict, so loading a v13 checkpoint
        # with use_cyclical_time=False (or any other config) cannot cause
        # a state-dict mismatch. The flag only controls whether the
        # embeddings are ADDED to user_dense_tok in forward().
        ...
        # v13: unconditional cyclical-time embedding tables. Small init.
        ...
        nn.init.normal_(self.month_embedding.weight, std=0.01)
