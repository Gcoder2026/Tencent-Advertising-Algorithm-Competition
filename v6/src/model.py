...
class ModelInput(NamedTuple):
    ...
    # v6: continuous log(1 + time_diff) per sequence event [B, L] float32.
    # Optional with default None for back-compat: when use_continuous_time
    # is False the model ignores it entirely.
    seq_time_log_dt: Optional[dict] = None
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # v6: continuous log(1+dt) time encoding (bug-fixed from v4).
        # Adds a small, bias-free linear projection of log(1+dt) to each
        # sequence token, additive to the existing discrete bucket
        # embedding. bias=False ensures log_dt=0 (padding) -> exactly zero
        # output, fixing v4's LayerNorm-bias-at-padding leak. Small init
        # std=0.01 keeps the contribution from dominating the trained
        # bucket signal early in training.
        ...
        if use_continuous_time:
            self.continuous_time_proj = nn.Linear(1, d_model, bias=False)
            nn.init.normal_(self.continuous_time_proj.weight, std=0.01)
        ...
        # User dense feature projection (if available). v6: deeper MLP.
        # Rationale: ~755 dims of pre-trained user dense embeddings are
        # being squeezed into a single d_model=64 token via one Linear
        # in the pcvr base. That's massive info compression. The 2-layer
        # MLP (Linear -> SiLU -> Dropout -> Linear -> LN) gives the
        # projection real capacity without changing the NS token layout
        # (still one user_dense token, so the rank_mixer T constraint
        # is unchanged).
    ...
    def _embed_seq_domain(
        ...
        time_log_dt: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        # v6: continuous log(1+dt) projection. Padding positions have
        # log_dt=0 (set in data.py); with bias=False, Linear(0)=0, so
        # padding contributes exactly the zero vector. Small init std=0.01
        # keeps initial magnitude small so this doesn't overpower the
        # trained bucket embedding above.
        if self.use_continuous_time and time_log_dt is not None:
            cont_in = time_log_dt.to(token_emb.dtype).unsqueeze(-1)  # (B, L, 1)
            token_emb = token_emb + self.continuous_time_proj(cont_in)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        for domain in self.seq_domains:
            time_log_dt = (
                inputs.seq_time_log_dt[domain]
                if inputs.seq_time_log_dt is not None and domain in inputs.seq_time_log_dt
                else None
            )
