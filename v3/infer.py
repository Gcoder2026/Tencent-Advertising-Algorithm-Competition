...
def main() -> None:
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(loader):
            ...
            # Select the conversion head (head 0) explicitly. With
            # action_num=1 head 0 is the only head; with action_num >= 2
            # (multi-task models) head 0 is the leaderboard target by
            # convention. The previous `squeeze(-1)` only removed singleton
            # dims and would silently leak the auxiliary click head into
            # the submitted score for action_num=2 models.
            conv_logits = logits[..., 0]
            prob = torch.sigmoid(conv_logits).detach().cpu().float()
