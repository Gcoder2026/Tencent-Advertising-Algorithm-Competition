...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--focal_gamma', type=float, default=2.0,
                        help='Focal Loss focusing parameter gamma '
                             '(effective only when --loss_type=focal)')
    parser.add_argument('--aux_click_weight', type=float, default=0.0,
                        help='Weight on the auxiliary click-prediction head loss. '
                             'Effective only when --action_num >= 2; the second '
                             'logit then receives a BCE loss against label_click '
                             '(label_type in {1,2}). 0 = disable multi-task.')
...
def main() -> None:
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        aux_click_weight=args.aux_click_weight,
