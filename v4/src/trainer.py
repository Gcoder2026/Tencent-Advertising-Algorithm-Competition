...
def _make_model_input(device_batch: Dict[str, Any], device) -> ModelInput:
    ...
    for d in seq_domains:
        ...
        # Continuous log(1 + dt) per event. Older datasets/checkpoints that
        # don't emit this key still work: the model side guards on
        # use_continuous_time before consuming it.
        key_tld = f"{d}_time_log_dt"
        if key_tld in device_batch:
            seq_tld[d] = device_batch[key_tld]
    return ModelInput(
        ...
        seq_time_log_dt=seq_tld if seq_tld else None,
    )
