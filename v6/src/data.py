...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._buf_seq = {}
        self._buf_seq_tb = {}
        self._buf_seq_tld = {}  # continuous log(1 + time_diff) per sequence event (v6)
        self._buf_seq_lens = {}
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for domain in self.seq_domains:
            ...
            # Time bucketing + continuous log-delta encoding (v6).
            ...
            if ts_ci is not None:
                ...
                # Continuous log(1 + dt). At padding positions ts_padded==0,
                # so we set log_dt=0 explicitly. Combined with v6's
                # bias-free Linear in continuous_time_proj, this means
                # padding emits exactly the zero vector (no LayerNorm
                # bias leak as in v4).
