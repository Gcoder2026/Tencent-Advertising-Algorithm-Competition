...
class ModelInput(NamedTuple):
    ...
    # v7/C3: continuous log(1 + time_diff) per event, [B, L] float32.
    # Optional (default None) for back-compat with v1-style ModelInput usage.
    seq_log_time_delta: Optional[dict] = None


class ContinuousTimeEncoder(nn.Module):
    """v7/C3 replacement for the bucket time embedding.
    ...
    construction:
      ...
      1. bias=False → Linear(0) = 0 exactly. With log_time_delta=0 at
         padding positions (set in dataset.py), the encoder output at
         padding is the exact zero vector — no LayerNorm-bias leak.
      2. An explicit multiplicative mask (log_time_delta != 0) on the
         output as a second line of defence.

    Small init std=0.01 keeps initial magnitude low so the rest of the
    model isn't perturbed before this projection has learned anything.
    """
...
class DINQueryGenerator(nn.Module):
    """v8/C1: DIN-style target-aware query generator.

    Replaces MultiSeqQueryGenerator's mean-pooling with target-attended
    aggregation per sequence. For each sequence token, computes an
    attention score against the target item embedding (mean of item NS
    tokens), then aggregates the sequence by these weights — so user
    history is summarized THROUGH the lens of the specific item being
    scored, rather than as a global average.

    Output signature is identical to MultiSeqQueryGenerator (list of
    (B, Nq, D) tensors of length S). Only difference is the additional
    item_ns input to forward(). This is C1 per roadmap-v3.

    Reference: Zhou et al. 2018 "Deep Interest Network for Click-Through
    Rate Prediction" (DIN).
    """
    ...
    def __init__(
        ...
        # Per-sequence DIN attention scoring MLP. Input is the classic
        # DIN feature concat [target, seq, target*seq, target-seq] of
        # dimension 4*d_model; output is a scalar score per sequence
        # token. Two layers: 4D -> D -> 1.
        self.attn_score_mlps = nn.ModuleList([
            nn.Sequential(
        ...
        # Same global-info + per-sequence FFNs as MultiSeqQueryGenerator.
        # We only replace the seq aggregation; the downstream Nq query
        # projection is unchanged so the rest of the model is undisturbed.
        global_info_dim = (num_ns + 1) * d_model
        self.global_info_norm = nn.LayerNorm(global_info_dim)
        self.query_ffns_per_seq = nn.ModuleList([
            nn.ModuleList([
    ...
    def forward(
        ...
        """Generates target-aware query tokens for each sequence.

        Args:
            ns_tokens: (B, M, D), all NS tokens.
            seq_tokens_list: List of (B, L_i, D) tensors, length S.
            seq_padding_masks: List of (B, L_i) masks. True = padding.
            item_ns: (B, num_item_ns, D) — the item-side NS tokens that
                define the target for attention. Mean-pooled here to
                produce a single target embedding per row.

        Returns:
            List of (B, Nq, D) query token tensors, length S — same
            output signature as MultiSeqQueryGenerator.
        """
        B = ns_tokens.shape[0]
        D = self.d_model
        ns_flat = ns_tokens.view(B, -1)

        # Target embedding: mean of item NS tokens. Shape (B, D).
        target_emb = item_ns.mean(dim=1)

        q_tokens_list = []
        for i in range(self.num_sequences):
            seq = seq_tokens_list[i]                       # (B, L, D)
            valid_mask = ~seq_padding_masks[i]             # (B, L), True = valid
            _, L, _ = seq.shape

            # Expand target across the sequence length.
            target_exp = target_emb.unsqueeze(1).expand(B, L, D)  # (B, L, D)

            # Classic DIN attention features.
            attn_in = torch.cat(
                [target_exp, seq, target_exp * seq, target_exp - seq],
                dim=-1,
            )  # (B, L, 4D)
            scores = self.attn_score_mlps[i](attn_in).squeeze(-1)  # (B, L)

            # Mask padding: large-negative score → ~0 weight after softmax.
            # If a row has zero valid tokens softmax becomes uniform over
            # padded zeros, so we explicitly zero the output below.
            scores = scores.masked_fill(~valid_mask, float('-inf'))
            any_valid = valid_mask.any(dim=-1, keepdim=True)  # (B, 1)
            attn_weights = F.softmax(scores, dim=-1)
            # NaN guard: rows with no valid tokens produce NaN softmax;
            # set those rows to zero attention weights.
            attn_weights = torch.where(
                any_valid, attn_weights, torch.zeros_like(attn_weights)
            )

            # Weighted sum → target-aware sequence summary (B, D).
            seq_pooled = (seq * attn_weights.unsqueeze(-1)).sum(dim=1)

            # Same downstream pipeline as MultiSeqQueryGenerator from here.
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # v7/C3: when True, REPLACES the discrete bucket time embedding
        # with a bias-free ContinuousTimeEncoder. Mutually exclusive
        # with the bucket path; bucket embedding is not constructed.
        use_continuous_time: bool = False,
        # v8/C1: when True, use DINQueryGenerator (target-aware attention
        # aggregation) instead of MultiSeqQueryGenerator (mean-pooling).
        # Roadmap-v3 card C1; expected +0.005 to +0.015 over the
        # mean-pool baseline. The two generators share output signature
        # so downstream HyFormer blocks are unaffected.
        use_din_query: bool = False,
    ) -> None:
        super().__init__()
        ...
        self.use_din_query = use_din_query
        ...
        # v7/C3: use_continuous_time REPLACES the bucket embedding with a
        ...
        # ================== HyFormer Components ==================
        # Query generator: target-aware (DIN, v8/C1) or mean-pool (v0..v7).
        # Both produce the same output signature so downstream blocks
        # don't care which one fired.
        if use_din_query:
            self.query_generator = DINQueryGenerator(
                d_model=d_model,
                num_ns=self.num_ns,
    ...
    def _init_params(self) -> None:
        ...
        # ContinuousTimeEncoder weights init in its own __init__ (std=0.01).
    ...
    def _embed_seq_domain(
        ...
        # v7/C3: branch on use_continuous_time. Mutually exclusive with the
        ...
        # zero-mask at padding (no silent fallback to zeros if missing).
        if self.use_continuous_time:
            if log_time_delta is None:
                raise RuntimeError(
                    "use_continuous_time=True requires log_time_delta to be "
                    "provided. Trainer/infer must populate seq_log_time_delta "
                    "in ModelInput — silent fallback to zeros was the v4 bug.")
